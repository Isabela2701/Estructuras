...
int main() {
    ...
    function<int(int, int, int)> noCola1 = [&](int k, int i, int j) -> int {
        if(k >= p) return 0;
        if(i >= f) return noCola1(k + 1, 0, 0);
        if(j >= c) return noCola1(k, i + 1, 0);
        
        int val = *(*(*(mat + k) + i) + j);
        int resultado = noCola1(k, i, j + 1);
        
        return (val * resultado) + val;
    };
    
    cout << "No Cola 1 - Transformacion: " << noCola1(0, 0, 0) << endl;
